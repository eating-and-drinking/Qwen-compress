...
假设我们有一个 **Qwen2-7B（教师，48层）→ Qwen2-1.8B（学生，24层）** 的蒸馏任务：

### 阶段0: 教师分解

```
教师层索引:  0  1  2  3  4  5 | 6  7  ... 47
能量信号:    [低][高][低][高][低][峰值]...[峰值]
             ↓
断点检测:    breakpoints = [5, 12, 18, 24, 30, 35, 39, 42, 44, 46, 47]
             ↓
功能组:      G0=[0-4], G1=[5-11], G2=[12-17], ..., G11=[47]
             ↓
组表示:      g_0^T, g_1^T, ..., g_11^T  (12个向量)
```
...
```
学生层:     S0 S1 S2 ... S23  (24层)
教师组:     G0 G1 G2 ... G11  (12组)
             ↓
代价矩阵 C: [24×12] 其中 C[l,k] = ||h_l^S - g_k^T||²
             ↓
Sinkhorn求解传输计划 γ:
γ = [
    [0.8, 0.2, 0, ..., 0],   # S0 主要对齐 G0，少量对齐 G1
    [0.3, 0.6, 0.1, ..., 0], # S1 对齐 G0/G1
    ...
    [0, 0, ..., 0.7, 0.3]    # S23 对齐 G10/G11
]
             ↓
期望位置 μ: [0.2, 0.8, 1.3, ..., 10.8, 11.3]
             ↓
单调约束:   μ_0 ≤ μ_1 ≤ ... ≤ μ_23
```

**关键点**：学生的第 l 层不一定只对齐一个教师组，而是通过最优传输获得**软分配**，同时**单调约束**确保学生层按顺序学习教师的功能。
